...
export function FilterSection({
  ...
  return (
    <section className="mb-8 animate-[fadeIn_0.5s_ease-out_0.1s_both] rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      {/* Search */}
      <div className="mb-4 flex gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border-[1.5px] border-[var(--color-border)] bg-[var(--color-bg-primary)] px-4 py-3 pl-11 text-[15px] text-[var(--color-text-primary)] transition-all duration-200 placeholder:text-[var(--color-text-tertiary)] focus:border-[var(--color-brand-primary)] focus:bg-[var(--color-bg-secondary)] focus:shadow-[0_0_0_3px_rgba(245,158,11,0.1)] focus:outline-none"
          />
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {displayedTags.map((tag) => (
              <button
                ...
                className={`flex items-center gap-2 rounded-md border-[1.5px] px-4 py-2 text-sm font-medium transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] ${
                  selectedTags.has(tag.name)
                    ? 'border-[var(--color-brand-primary)] bg-[var(--color-brand-primary)] font-semibold text-white shadow-[0_2px_8px_rgba(245,158,11,0.3)]'
                    : 'border-[var(--color-border)] bg-[var(--color-bg-secondary)] text-[var(--color-text-primary)] hover:border-[var(--color-brand-primary)]'
                }`}
              >
                <span>{tag.name}</span>
                <span className={`rounded-[10px] px-2 py-0.5 text-xs font-semibold ${
                  selectedTags.has(tag.name)
                    ? 'bg-white/25'
        ...
        <div className="mt-4 flex flex-wrap gap-2 border-t border-[var(--color-border)] pt-4 animate-[slideIn_0.2s_ease-out]">
          {activeFilters.map((filter, index) => (
            <div
              ...
              className="flex items-center gap-2 rounded-md bg-[var(--color-brand-accent)] px-3 py-1.5 text-[13px] font-semibold text-[#92400E] shadow-[0_2px_4px_rgba(245,158,11,0.3)] dark:text-[#78350F]"
            >
