...
| 分野 | 技術 | 選定理由 |
|------|------|----------|
| **Frontend** | Next.js (App Router)<br>React<br>TypeScript | 実務で使い慣れたスタック。個人プロジェクトで最新バージョン（Next.js 16 canary / React 19）を試したかった。App Routerは実務でも使い始めており、Server Componentsの理解を深める目的もある。 |
| **Styling** | Tailwind CSS | 実務で使い慣れており、v4の新機能を試す目的で採用。 |
| **Cache / DB** | Upstash Redis | Vercelのサーバーレス環境ではインメモリキャッシュが効かないため、外部KVストアが必要だった。サーバーレス対応かつ無料枠があるUpstashを採用。 |
| **Translation** | Google Cloud Translation API | Hacker Newsの英語記事タイトルを自動翻訳。結果はRedisに30日キャッシュしてコスト最適化。 |
| **Package Manager** | pnpm | 高速かつ安定したパッケージ管理。 |
| **Testing** | Vitest<br>React Testing Library<br>Playwright<br>Storybook | 実務ではJest/RSpecを使用。個人プロジェクトでPlaywright（E2E）とStorybook（コンポーネントカタログ）を試したかった。VitestはESM構成（`"type": "module"`）との相性が良く、StorybookもViteベースのためツールチェーンが統一できる。 |
