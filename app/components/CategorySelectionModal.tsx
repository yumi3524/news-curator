...
function CategorySelectionModalContent({
  ...
  const handleCategoryToggle = (categoryId: CategoryId) => {
    setSelectedCategories((prev) => toggleArrayItem(prev, categoryId));
  };
  ...
  return (
    <Modal
      icon={<Sparkles className="h-4 w-4 text-[var(--color-brand-primary)]" />}
      title={isOnboarding ? 'カテゴリを選択' : 'カテゴリ設定'}
      description={isOnboarding ? '興味のある分野を選んでください' : '表示する記事のカテゴリを選択'}
